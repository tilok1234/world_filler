...
/**
 * Render the analysis bundle as 1px-per-cell PNGs: a semantic terrain base
 * plus heatmap overlays blended onto a dimmed base. Colors are World
 * Filler's own inspection palette — renders are evidence, not contract.
 */

type Rgb = readonly [number, number, number];

const MATERIAL_COLORS: Readonly<Record<string, Rgb>> = {
...
const FALLBACK_COLOR: Rgb = [200, 60, 200];
...
export function renderTerrain(model: WorldModel): Uint8Array {
...
function dimmedBase(terrain: Readonly<Uint8Array>): Uint8Array {
  const base = new Uint8Array(terrain.length);
  for (let i = 0; i < terrain.length; i += 4) {
    base[i] = Math.floor((terrain[i] as number) / 3);
    base[i + 1] = Math.floor((terrain[i + 1] as number) / 3);
    base[i + 2] = Math.floor((terrain[i + 2] as number) / 3);
    base[i + 3] = 255;
  }
  return base;
}
...
function overlayMask(base: Readonly<Uint8Array>, mask: Readonly<Uint8Array>, color: Rgb): Uint8Array {
  const rgba = Uint8Array.from(base);
...
function overlayLabels(base: Readonly<Uint8Array>, labels: Readonly<Int32Array>): Uint8Array {
  const rgba = Uint8Array.from(base);
...
export function upscaleRgba(rgba: Readonly<Uint8Array>, width: number, height: number, factor: number): Uint8Array {
  if (!Number.isInteger(factor) || factor < 1 || factor > 16) {
    throw new Error(`render: scale factor must be an integer in [1, 16], got ${factor}`);
  }
  if (factor === 1) return Uint8Array.from(rgba);
...
export function renderAnalysis(model: WorldModel, bundle: AnalysisBundle, scale: number = 1): RenderedMap[] {
  const { width, height } = model.dimensions;
  ...
  const layers: Array<{ name: string; rgba: Uint8Array }> = [
