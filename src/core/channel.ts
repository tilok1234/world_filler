...
export class Channel {
  ...
  at(...names: readonly string[]): Channel {
    let current: Channel = this;
    for (const name of names) current = current.child(name);
    return current;
  }
  ...
  pick<T>(items: readonly T[], index: number): T {
    if (items.length === 0) throw new Error("channel: cannot pick from an empty list");
    return items[this.int(index, items.length)] as T;
  }
   ...
   * capped total.
   */
  weightedIndex(weights: readonly number[], index: number): number {
    let total = 0;
   ...
   */
  shuffle<T>(items: readonly T[], baseIndex: number = 0): T[] {
    const result = [...items];
