...
function printedJson(stdout, open, close) {
...
describe("iteration verbs", () => {
    ...
    it("unlock prints the remaining locks and refuses unknown ids", () => {
        ...
        try {
            const raw = JSON.parse(readFileSync(RECIPE, "utf8"));
            const lock = {
...
describe("the full direct -> lock -> reroll -> export cycle", () => {
    it("runs end to end through documented commands; the lock survives the reroll byte-stably", () => {
        ...
        try {
            ...
            const doc = JSON.parse(readFileSync(join(placeOut, "placements.json"), "utf8"));
            ...
            const lockEntry = printedJson(locked.stdout, "{", "}");
            ...
            const after = JSON.parse(readFileSync(join(packOut, "placements.json"), "utf8"));
