...
import { WorldModel, ALL_LADDER_RUNGS } from "../src/world/model.js";
...
describe("walkability parity", () => {
    ...
    const expectedCoverage = JSON.parse(readFileSync(join(root, "fixtures", "expected-coverage.json"), "utf8"));
    for (const dir of committed) {
        ...
        it(`matches the reference grid bit-for-bit: ${name}`, () => {
            ...
            assert.deepEqual(parity.rungCounts, expected, `${name}: rung coverage moved — upstream behavior change? Review before re-recording.`);
        });
    }
    it("exercises every ladder rung across committed fixtures, or documents the gap", () => {
        const documentedGaps = JSON.parse(readFileSync(join(root, "fixtures", "expected-coverage.json"), "utf8")).__documentedGaps ?? [];
        const union = new Set(documentedGaps);
        for (const dir of committed) {
            const name = basename(dir);
        ...
        assert.deepEqual(missing, [], `rungs neither exercised by a committed fixture nor listed in __documentedGaps: ${missing.join(", ")}`);
    });
