...
function withTamperedPack(mutate, run) {
...
function editJson(dir, name, edit) {
...
describe("pack reading refusals", () => {
    ...
    it("refuses a missing payload file", () => {
        withTamperedPack((dir) => rmSync(join(dir, "minimap.png")), (dir) => {
            assert.throws(() => readGamePack(dir), /minimap\.png listed in manifest\.files is missing/);
        });
    });
    it("refuses an unknown packFormat", () => {
        withTamperedPack((dir) => editJson(dir, "manifest.json", (manifest) => { manifest["packFormat"] = 2; }), (dir) => {
            assert.throws(() => readGamePack(dir), /unsupported packFormat 2/);
        });
    });
    it("refuses an unknown artifactFormat", () => {
        withTamperedPack((dir) => editJson(dir, "manifest.json", (manifest) => { manifest["artifactFormat"] = 7; }), (dir) => {
    ...
    it("refuses when baseArtifactSha256 disagrees with the world.json hash", () => {
        withTamperedPack((dir) => editJson(dir, "manifest.json", (manifest) => { manifest["baseArtifactSha256"] = "0".repeat(64); }), (dir) => {
    ...
    it("refuses walkability format or summary drift", () => {
        withTamperedPack((dir) => {
            editJson(dir, "manifest.json", (manifest) => {
                const walkability = manifest["walkability"];
                walkability["floodCount"] = walkability["floodCount"] + 1;
            });
        }, (dir) => {
...
describe("artifact gates", () => {
    it("refuses a wrong formatVersion", () => {
        ...
        assert.throws(() => new WorldModel(artifact), (error) => {
