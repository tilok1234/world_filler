...
describe("spatial analysis", () => {
    ...
    it("passes spot asserts on a real world", () => {
        ...
        for (let i = 0; i < width * width; i += 1) {
            if (bundle.distanceFromRoads[i] === 0)
                roadZero += 1;
            if (bundle.corridor[i] === 1 && bundle.bits[i] !== 1)
                corridorOnWalkable = false;
            const clear = bundle.clearance[i];
            if (bundle.bits[i] === 1 && clear < 1)
                clearanceConsistent = false;
            if (bundle.bits[i] === 0 && clear !== 0)
                clearanceConsistent = false;
        }
        ...
        const spawnSize = bundle.summary.components.sizes[spawnComponent];
