...
import { solvePlacements, PlacementError } from "../src/place/solver.js";
...
function solveFor(packName, recipeRaw) {
...
describe("placement solver", () => {
    ...
    it("places every fixture budget or explains the failure", () => {
        for (const packName of ["fen-hollow", "dust-hollow", "tiny-temperate"]) {
            ...
            for (const region of plan.regions) {
                if (region.budgets.dungeonBindings <= 0)
                    continue;
                const placedHere = doc.placements.filter((placement) => placement.rule === "dungeon_binding.v1" && placement.regionId === region.id).length;
                const failedHere = doc.failures.some((failure) => failure.rule === "dungeon_binding.v1" && failure.regionId === region.id);
                assert.ok(placedHere === region.budgets.dungeonBindings || failedHere, `${packName}/${region.id}: dungeon budget places fully or explains`);
            }
    ...
    it("never places on unwalkable or spawn-unreachable cells, and reservations respect invariants", () => {
        ...
        for (const placement of doc.placements) {
            ...
            if (placement.rule === "world_boss.v1") {
                ...
                const side = placement.arenaSide;
        ...
        for (const a of doc.placements) {
            for (const b of doc.placements) {
                ...
                if (a.rule === "world_boss.v1") {
                    const [ox, oy] = a.arenaOrigin;
                    for (let y = oy; y < oy + a.arenaSide; y += 1) {
                        for (let x = ox; x < ox + a.arenaSide; x += 1)
                            physical.push([x, y]);
                    }
                }
                else {
                ...
                for (const [px, py] of physical) {
                    ...
                    assert.ok(dx * dx + dy * dy > r2, `${a.id} physical cell (${px}, ${py}) sits inside the exclusion disc of ${b.id}`);
                }
            }
        }
    });
    ...
    it("rerolling one uncoupled region leaves the other region byte-identical", () => {
        ...
        assert.equal(canonicalJson(secondAfter), canonicalJson(second), "the uncoupled region's placement must be byte-identical across the reroll");
    });
    ...
    it("explanations carry channel, funnel, score terms, and the chosen candidate", () => {
        ...
        for (const placement of doc.placements) {
            ...
            assert.equal(placement.score, placement.scoreTerms.reduce((sum, term) => sum + term.contribution, 0), "score equals the sum of its terms");
            assert.ok(placement.topCandidates.some((candidate) => candidate.cell[0] === placement.cell[0] && candidate.cell[1] === placement.cell[1]), "chosen cell appears in the top-candidate window");
        }
    });
    ...
    it("rejects rerolls referencing unknown regions", () => {
        ...
        const plan = compilePlan(model, bundle, recipe);
        assert.throws(() => solvePlacements(model, bundle, plan, recipe), (error) => {
