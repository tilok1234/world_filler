...
const golden = JSON.parse(readFileSync(join(repoRoot(), "fixtures", "golden", "kernel.json"), "utf8"));
describe("hash kernel golden vectors", () => {
    it("mix32", () => {
        for (const [input, expected] of golden.mix32)
            assert.equal(mix32(input), expected);
    });
    it("combine32", () => {
        for (const [a, b, expected] of golden.combine32)
            assert.equal(combine32(a, b), expected);
    });
    ...
    it("fold53", () => {
        for (const [input, expected] of golden.fold53)
            assert.equal(fold53(input), expected);
    ...
    it("channel derivation chain", () => {
        ...
        for (const expected of recorded) {
            const segment = expected.path.split("/").pop();
    ...
    it("indexed draws", () => {
        ...
        assert.deepEqual(Array.from({ length: 8 }, (_, i) => channel.weightedIndex([45, 30, 20, 5], i)), golden.draws.weighted_45_30_20_5);
