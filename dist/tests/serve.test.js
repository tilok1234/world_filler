...
describe("director studio API", () => {
    it("serves the UI page and discovers the fixture worlds", async () => {
        ...
        const names = data.worlds.map((entry) => entry.name);
        assert.ok(names.includes("fen-hollow") && names.includes("dust-hollow"));
        const fen = data.worlds.find((entry) => entry.name === "fen-hollow");
    ...
    it("directs a world byte-identically to the CLI export", async () => {
        ...
        assert.ok(data.gates.every((gate) => gate.status !== "fail"));
        ...
        for (const name of ["manifest.json", "content-plan.json", "placements.json", "territories.json", "report.json", "view.html"]) {
            assert.equal(readFileSync(join(serverOut, name), "utf8"), readFileSync(join(cliOut, name), "utf8"), `${name} byte-identical between server direct and CLI export`);
        }
    });
    ...
    it("lock, reroll, and unlock write valid recipe edits that survive a re-direct", async () => {
        const pack = await api("GET", "/api/pack?world=fen-hollow");
        const boss = pack.data.placements.placements.find((entry) => entry.rule === "world_boss.v1");
        ...
        const other = pack.data.placements.placements.find((entry) => entry.regionId !== boss.regionId);
        ...
        const heldBoss = after_.data.placements.placements.find((entry) => entry.id === boss.id);
