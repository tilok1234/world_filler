...
function runPipeline(recipeOverride) {
...
function expectRefusal(name, tamper, messagePattern) {
    const base = mkdtempSync(join(tmpdir(), "wf-freeze-"));
    try {
        const dir = join(base, "pack");
        writeContentPack(built, dir);
        tamper(dir);
        assert.throws(() => verifyContentPack(FEN, dir), (error) => {
...
function rehash(dir, names) {
    const manifest = JSON.parse(readFileSync(join(dir, "manifest.json"), "utf8"));
    for (const name of names)
        manifest.files[name] = sha256Hex(readFileSync(join(dir, name)));
...
describe("freeze review: exporter cross-document identity", () => {
    it("refuses a passing report that audited a different recipe", () => {
        ...
        assert.throws(() => buildContentPack({
            model: pipeline.model,
            worldName: "fen-hollow",
            baseArtifactSha256: pipeline.baseArtifactSha256,
            recipe: pipeline.recipe,
...
describe("freeze review: recipe lock id grammar", () => {
    it("refuses lock ids that violate the frozen placement id scheme", () => {
        ...
        for (const id of [
            ...
            assert.throws(() => normalizeRecipe({ ...raw, locks: { placements: [{ ...lockBase, id }] } }), (error) => {
...
describe("freeze review: golden pack pins the frozen serialization", () => {
    it("a fresh build byte-matches fixtures/golden/content-pack", () => {
        ...
        for (const [name, bytes] of built.files) {
            assert.equal(bytes, readFileSync(join(goldenDir, name), "utf8"), `${name} must byte-match the golden fixture (re-record via node dist/tools/recordGoldenPack.js — an explicit, logged decision)`);
        }
        assert.equal(canonicalJson(built.manifest), readFileSync(join(goldenDir, "manifest.json"), "utf8"), "manifest.json must byte-match the golden fixture");
    });
});
