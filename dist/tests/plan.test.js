...
describe("DirectorRecipe normalization", () => {
    ...
    it("rejects unknown vocabulary with named errors", () => {
        ...
        assert.throws(() => normalizeRecipe({ ...MINIMAL, base: { generationIdentitySha256: "zz" } }), /64-hex sha256/);
        assert.throws(() => normalizeRecipe({ ...MINIMAL, danger: { overrides: [{ regionId: "r", band: 9 }] } }), /band must be an integer in \[0, 4\]/);
        assert.throws(() => normalizeRecipe({
            ...MINIMAL,
            danger: { overrides: [{ regionId: "r", band: 1 }, { regionId: "r", band: 2 }] },
        }), /duplicate danger override/);
        assert.throws(() => normalizeRecipe({ ...MINIMAL, dungeonAnchors: { poiTypes: ["prop.oak"] } }), /poi\.\* type names/);
    });
});
describe("regional content plan", () => {
    ...
    it("produces sane bands, budgets, and waivers on a real world", () => {
        ...
        for (const region of plan.regions) {
            ...
            if (region.regionClass === "minor") {
                assert.deepEqual(region.budgets, { territories: 0, encounterSites: 0, dungeonBindings: 0, worldBosses: 0 }, `${region.id} is minor but carries budget`);
    ...
    it("flags progression traps created by overrides, and rejects unknown override regions", () => {
        ...
        assert.throws(() => compilePlan(islandModel, islandBundle, unknown), (error) => {
    ...
    it("waives unreachable regions off the spawn component", () => {
        ...
        for (let y = 0; y < 8; y += 1) {
            for (let x = 3; x < 5; x += 1)
                setMaterial(artifact, x, y, "water.deep");
        }
        const islandModel = new WorldModel(artifact);
        const islandBundle = analyzeWorld(islandModel);
