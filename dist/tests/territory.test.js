...
import { growTerritories, encodeRuns, decodeRuns } from "../src/territory/territory.js";
...
function territoriesFor(packName, recipeRaw) {
...
describe("spawn territories", () => {
    ...
    it("holds every ground invariant on all fixtures", () => {
        for (const packName of ["fen-hollow", "dust-hollow", "tiny-temperate"]) {
            ...
            for (let t = 0; t < doc.territories.length; t += 1) {
                const territory = doc.territories[t];
    ...
    it("meets coverage budgets or explains, and rosters resolve in the library", () => {
        ...
        for (const region of doc.coverage.regions) {
            assert.ok(region.emitted === region.budget ||
                doc.failures.some((failure) => failure.regionId === region.regionId), `${region.regionId}: budget met or failure recorded`);
        }
    });
    ...
    it("fails with no_matching_roster when the library cannot cover a biome", () => {
        ...
        assert.equal(doc.failures[0].reason, "no_matching_roster");
    });
    it("rejects invalid content libraries with named errors", () => {
        assert.throws(() => normalizeRecipe({ ...MINIMAL, contentLibrary: { enemies: [] } }), /non-empty array/);
        ...
        assert.throws(() => normalizeRecipe({ ...MINIMAL, territoryRule: { respawnPressure: "extreme" } }), /respawnPressure must be one of/);
        assert.throws(() => normalizeRecipe({ ...MINIMAL, territoryRule: { minCells: 50, maxCells: 10 } }), /maxCells must be >= minCells/);
    });
});
