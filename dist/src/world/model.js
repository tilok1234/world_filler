import { CHUNK_LAYERS, } from "./types.js";
 ...
 */
const BLOCKED_MATERIALS = new Set(["water.deep", "terrain.rock", "terrain.swamp"]);
const STRUCTURE_PASS_CELLS = {
...
export class WorldModel {
    ...
    constructor(raw) {
            ...
            for (const layer of CHUNK_LAYERS) {
                ...
                for (let ly = 0; ly < chunkHeight; ly += 1) {
                    ...
                    for (let lx = 0; lx < chunkWidth; lx += 1) {
                        flat[(originY + ly) * width + originX + lx] = row[lx];
                    }
                }
            }
        }
        ...
        const corridorAt = (index) => CORRIDOR_MATERIALS.has(palette[material[index]]);
        const riverAt = (index) => river[index] !== 0;
        const bridged = (index, stride, room, room2) => {
            if (!room)
                return false;
            ...
            if (!room2)
                return false;
            if (before && riverAt(index + stride) && corridorAt(index + 2 * stride))
                return true;
            if (after && riverAt(index - stride) && corridorAt(index - 2 * stride))
                return true;
            return false;
        };
    ...
    layerValueAt(layer, x, y) {
        return this.layers[layer][this.index(x, y)];
    }
    materialAt(x, y) {
        return this.raw.semanticPalette[this.layerValueAt("material", x, y)];
    }
    structureAt(x, y) {
        const value = this.layerValueAt("structure", x, y);
        return value === 0 ? null : this.raw.structureTypes[value - 1];
    }
    propAt(x, y) {
        const value = this.layerValueAt("prop", x, y);
        return value === 0 ? null : this.raw.propTypes[value - 1];
    }
    fenceAt(x, y) {
        const value = this.layerValueAt("fence", x, y);
        return value === 0 ? null : this.raw.fenceTypes[value - 1];
    }
    pierAt(x, y) {
        const value = this.layerValueAt("pier", x, y);
        return value === 0 ? null : this.raw.pierTypes[value - 1];
    }
    trailAt(x, y) {
    ...
    classifyCell(x, y) {
        ...
        if (this.trailAt(x, y))
            return "trail_walk";
        if (this.pierAt(x, y) !== null)
            return "pier_walk";
        const index = this.index(x, y);
        if (this.routeCrossingCells.has(index))
            return "crossing_route_walk";
        if (this.streetFordCells.has(index))
            return "crossing_street_ford_walk";
        const tier = this.riverTierAt(x, y);
        if (tier === 1)
            return "river_stream_block";
        if (tier > 1)
            return "river_major_block";
        const material = this.materialAt(x, y);
        if (BLOCKED_MATERIALS.has(material)) {
            if (material === "water.deep")
                return "material_block_deep_water";
    ...
    deriveWalkability() {
        ...
        const rungCounts = Object.fromEntries(ALL_LADDER_RUNGS.map((rung) => [rung, 0]));
        for (let y = 0; y < height; y += 1) {
            for (let x = 0; x < width; x += 1) {
                const rung = this.classifyCell(x, y);
                rungCounts[rung] += 1;
                if (WALKING_RUNGS.has(rung))
                    bits[y * width + x] = 1;
            }
        }
        ...
        for (let radius = 0; radius < 8; radius += 1) {
            for (let dy = -radius; dy <= radius; dy += 1) {
                for (let dx = -radius; dx <= radius; dx += 1) {
                    const x = cx + dx;
                    const y = cy + dy;
                    if (this.inBounds(x, y) && bits[y * width + x] === 1)
                        return [x, y];
                }
            }
        }
...
export function floodCount(bits, width, height, startIndex) {
...
function validateArtifact(input) {
    ...
    if (raw.formatVersion !== SUPPORTED_ARTIFACT_FORMAT) {
        throw new ArtifactError(`world.json: unsupported artifact format ${String(raw.formatVersion)}; this reader supports format ${SUPPORTED_ARTIFACT_FORMAT}`);
    }
    ...
    for (const records of ["destinations", "routes", "settlements", "landmarks", "regions"]) {
