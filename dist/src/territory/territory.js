...
export function encodeRuns(cells, width) {
 ...
 */
export function decodeRuns(runs, width) {
    const cells = new Set();
    for (const run of runs) {
        ...
        if (x < 0 || y < 0 || length < 1 || x + length > width) {
            throw new Error(`decodeRuns: run [${x}, ${y}, ${length}] is malformed (needs x >= 0, y >= 0, length >= 1, x + length <= width ${width})`);
        }
        for (let i = 0; i < length; i += 1)
            cells.add(y * width + x + i);
    }
    return cells;
}
function rosterFor(recipe, biome, band) {
    const matches = [];
    for (const enemy of recipe.contentLibrary.enemies) {
        if (band < enemy.minBand || band > enemy.maxBand)
            continue;
        if (!enemy.biomes.includes(biome))
            continue;
        matches.push({ enemyId: enemy.id, weightPercent: enemy.weightPercent, nightOnly: enemy.nightOnly });
    }
    return matches;
}
export function growTerritories(model, bundle, plan, placements, recipe) {
    ...
    for (const region of plan.regions) {
        ...
        if (budget <= 0)
            continue;
        const label = labelById.get(region.id);
        ...
        let eligibleCount = 0;
        for (let i = 0; i < width * height; i += 1) {
            if (bundle.regionLabels[i] !== label)
                continue;
            if (bundle.bits[i] !== 1)
                continue;
            if (bundle.distanceFromSpawn[i] === UNREACHABLE)
                continue;
            if (blocked[i] === 1 || claimed[i] === 1)
                continue;
            const x = i % width;
            const y = (i - x) / width;
            if (recipe.paint.noContent.some((zone) => x >= zone.rect[0] && x <= zone.rect[2] && y >= zone.rect[1] && y <= zone.rect[3]))
                continue;
            eligible[i] = 1;
            eligibleCount += 1;
        }
        const roster = rosterFor(recipe, region.biome, band);
        const targetSize = Math.max(rule.minCells, Math.min(rule.maxCells, Math.floor((region.hostileWalkableCells * rule.targetHostileCoveragePermille) / 1000 / budget)));
        ...
        for (let slot = 0; slot < budget; slot += 1) {
            ...
            for (let start = 0; start < width * height; start += 1) {
                if (eligible[start] !== 1 || pocketOf[start] !== -1)
                    continue;
            ...
            for (let i = 0; i < width * height; i += 1) {
                if (eligible[i] !== 1)
                    continue;
                if (pocketSizes[pocketOf[i]] < rule.minCells)
                    continue;
                const distance = bundle.distanceFromSettlements[i];
            ...
            const seedIndex = seeds[draw].index;
            // Deterministic BFS growth confined to eligible cells.
            const cells = new Set([seedIndex]);
            ...
            for (let head = 0; head < queue.length && cells.size < targetSize; head += 1) {
                ...
                for (const [dx, dy] of [[0, -1], [1, 0], [0, 1], [-1, 0]]) {
                    ...
                    if (nx < 0 || ny < 0 || nx >= width || ny >= height)
                        continue;
                    const next = ny * width + nx;
                    if (eligible[next] !== 1 || cells.has(next))
                        continue;
                    cells.add(next);
                    queue.push(next);
