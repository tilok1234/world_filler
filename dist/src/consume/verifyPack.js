...
import { CONTENT_PACK_FORMAT, PLACEMENTS_FORMAT, PLAN_FORMAT, REPORT_FORMAT, TERRITORIES_FORMAT, } from "../core/version.js";
/**
...
const REQUIRED_FILES = ["content-plan.json", "placements.json", "report.json", "territories.json"];
...
export function verifyContentPack(worldPackDir, contentPackDir) {
    let manifestBytes;
    try {
    ...
    if (manifest.packFormat !== CONTENT_PACK_FORMAT) {
        throw new VerifyError(`verify: unsupported packFormat ${String(manifest.packFormat)}; this build reads format ${CONTENT_PACK_FORMAT}`);
    }
    ...
    if (listed.join(",") !== REQUIRED_FILES.join(",")) {
        throw new VerifyError(`verify: manifest.files must list exactly ${REQUIRED_FILES.join(", ")} (got: ${listed.join(", ") || "nothing"})`);
    }
    ...
    const plan = parseJson("content-plan.json", payloadBytes.get("content-plan.json"));
    const placements = parseJson("placements.json", payloadBytes.get("placements.json"));
    const territories = parseJson("territories.json", payloadBytes.get("territories.json"));
    const report = parseJson("report.json", payloadBytes.get("report.json"));
    ...
    if (placements.placementsFormat !== PLACEMENTS_FORMAT) {
        throw new VerifyError(`verify: placements.json placementsFormat ${String(placements.placementsFormat)}; format 1 packs carry ${PLACEMENTS_FORMAT}`);
    }
    if (territories.territoriesFormat !== TERRITORIES_FORMAT) {
        throw new VerifyError(`verify: territories.json territoriesFormat ${String(territories.territoriesFormat)}; format 1 packs carry ${TERRITORIES_FORMAT}`);
    }
    ...
    if (base === undefined)
        throw new VerifyError("verify: manifest.base is missing");
    if (base.generationIdentitySha256 !== worldPack.manifest.generator.generationIdentitySha256) {
        throw new VerifyError(`verify: content pack was directed against base ${base.generationIdentitySha256} but the world pack is ` +
    ...
    for (const [name, doc] of [
        ...
        if (JSON.stringify(Object.entries(doc.rulePacks ?? {}).sort()) !==
    ...
    if (counts === undefined)
        throw new VerifyError("verify: manifest.counts is missing");
    if (counts.placements !== placements.placements.length ||
    ...
    const walkable = (x, y) => x >= 0 && y >= 0 && x < width && y < height && unpackBit(grid, y * width + x);
