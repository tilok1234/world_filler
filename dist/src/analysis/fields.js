...
export function distanceField(bits, width, height, sources) {
...
export function walkableComponents(bits, width, height) {
    const labels = new Int32Array(width * height).fill(-1);
    const sizes = [];
    for (let start = 0; start < width * height; start += 1) {
        if (bits[start] !== 1 || labels[start] !== -1)
            continue;
 ...
 */
export function clearanceField(bits, width, height) {
    ...
    for (let y = 0; y < height; y += 1) {
        for (let x = 0; x < width; x += 1) {
            ...
            const up = clearance[index - width];
            const left = clearance[index - 1];
            const upLeft = clearance[index - width - 1];
 ...
 */
export function corridorCells(bits, width, height) {
...
export function cellsAdjacentTo(bits, width, height, isSource) {
    const seeds = [];
    for (let y = 0; y < height; y += 1) {
        for (let x = 0; x < width; x += 1) {
            const index = y * width + x;
