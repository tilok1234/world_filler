 ...
 */
const VOID_MATERIALS = new Set(["water.deep", "water.shallow", "terrain.rock"]);
export function segmentRegions(model) {
    const { width, height } = model.dimensions;
    const labels = new Int32Array(width * height).fill(-1);
    const patches = [];
    const materialAtIndex = (index) => {
        const x = index % width;
    ...
    const neighborSets = patches.map(() => new Set());
    for (let y = 0; y < height; y += 1) {
        for (let x = 0; x < width; x += 1) {
            ...
            if (y < height - 1) {
                const south = labels[(y + 1) * width + x];
    ...
    const idOf = (patchIndex) => {
        const patch = patches[patchIndex];
        const short = patch.biome.replace(/^terrain\./, "").replace(/^water\./, "");
        return `region.${short}.${patch.anchorIndex}`;
    };
    const regions = patches.map((patch, index) => {
        const neighborIds = [...neighborSets[index]].map(idOf).sort();
        return {
            id: idOf(index),
            biome: patch.biome,
