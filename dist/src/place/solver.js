...
function regionChannel(state, regionId) {
...
function discCells(state, cx, cy, radius) {
    const cells = [];
...
function rectCells(state, x0, y0, w, h) {
    const cells = [];
...
function anySafe(state, cells) {
    for (const index of cells) {
        if (state.bundle.safeZone[index] === 1)
            return true;
    }
    return false;
}
function claim(mask, cells) {
    for (const index of cells)
        mask[index] = 1;
}
/**
 ...
 * physical cells.
 */
function admissible(state, physical, buffer) {
    if (anyIn(state.placementMask, physical))
        return false;
    if (anyIn(state.bufferMask, physical))
        return false;
    if (anyIn(state.placementMask, buffer))
        return false;
    return true;
}
/**
 ...
 */
function accessCellNear(state, cx, cy) {
    for (let radius = 0; radius <= ANCHOR_ACCESS_RADIUS; radius += 1) {
        for (let dy = -radius; dy <= radius; dy += 1) {
            for (let dx = -radius; dx <= radius; dx += 1) {
                ...
                if (state.bundle.bits[index] === 1 && state.bundle.distanceFromSpawn[index] !== UNREACHABLE) {
                    return [x, y];
                }
            }
        }
...
function placeDungeons(state, regionId, budget, anchorPois, takenIds) {
    ...
    for (const anchor of anchorPois) {
        ...
        const settlementDistance = bundle.distanceFromSettlements[access[1] * state.width + access[0]];
    ...
    for (let iteration = 0; iteration < budget; iteration += 1) {
        slotNumber += 1;
        while (takenIds.has(`placement.dungeon.${regionId}.${slotNumber}`))
            slotNumber += 1;
        const slot = slotNumber;
        const viable = [];
        for (const candidate of remaining) {
            const physical = rectCells(state, candidate.anchor.footprint.x, candidate.anchor.footprint.y, candidate.anchor.footprint.w, candidate.anchor.footprint.h);
            const buffer = discCells(state, candidate.anchor.cell[0], candidate.anchor.cell[1], recipe.dungeonRule.exclusionRadius);
            if (admissible(state, physical, buffer))
        ...
        const physical = rectCells(state, chosen.anchor.footprint.x, chosen.anchor.footprint.y, chosen.anchor.footprint.w, chosen.anchor.footprint.h);
    ...
    const chosenIds = new Set(state.placements
...
function placeBoss(state, regionId, slot, peerFields) {
    const { recipe, bundle } = state;
    const rule = recipe.worldBossRule;
    const label = state.labelById.get(regionId);
    ...
    for (let y = 0; y < state.height; y += 1) {
        for (let x = 0; x < state.width; x += 1) {
            ...
            if (bundle.clearance[anchorIndex] < side)
                continue;
            ...
            if (bundle.regionLabels[centerIndex] !== label)
                continue;
            inRegion += 1;
            if (bundle.distanceFromSpawn[centerIndex] === UNREACHABLE)
                continue;
            afterReachable += 1;
            if (state.inNoContent(centerX, centerY))
                continue;
            afterPaint += 1;
            const square = rectCells(state, originX, originY, side, side);
            if (anySafe(state, square) || anyIn(state.placementMask, square) || anyIn(state.bufferMask, square))
                continue;
            ...
            if (anyIn(state.placementMask, buffer))
                continue;
            afterExclusion += 1;
            const settlementDistance = bundle.distanceFromSettlements[centerIndex];
            const settlementValue = settlementDistance === UNREACHABLE ? 1000 : permilleOf(settlementDistance, settlementMax);
            if (settlementDistance !== UNREACHABLE && settlementDistance < rule.minSettlementPathDistance)
                continue;
            afterSettlement += 1;
            let peerOk = true;
            for (const field of peerFields) {
                const peerDistance = field[centerIndex];
            ...
            if (!peerOk)
                continue;
            const clearanceValue = permilleOf(Math.min(bundle.clearance[anchorIndex], side * 2), side * 2);
            const roadDistance = bundle.distanceFromRoads[centerIndex];
...
export function solvePlacements(model, bundle, plan, recipe) {
    ...
    const inRect = (rect, x, y) => x >= rect[0] && x <= rect[2] && y >= rect[1] && y <= rect[3];
    const state = {
        model,
        bundle,
        recipe,
        ...
        preferBonusAt: (x, y) => {
            ...
            for (const zone of recipe.paint.preferContent) {
                if (inRect(zone.rect, x, y) && zone.bonusPermille > best)
                    best = zone.bonusPermille;
            }
            return best;
        },
    };
    ...
    for (const poi of model.pois) {
            ...
            for (let dy = -radius; dy <= radius; dy += 1) {
                for (let dx = -radius; dx <= radius; dx += 1) {
                    ...
                    if (x < 0 || y < 0 || x >= width || y >= height)
                        continue;
                    const candidate = bundle.regionLabels[y * width + x];
                    if (candidate !== -1) {
                        label = candidate;
                        break outer;
                    }
        ...
        const regionId = bundle.regions[label].id;
    ...
    for (const list of anchorsByRegion.values())
        list.sort((a, b) => a.poiId - b.poiId);
    ...
    const heldByRegionRule = new Map();
    const regionBudget = (regionId, rule) => {
    ...
    for (const lock of recipe.locks.placements) {
        ...
        if (label === undefined)
            reasons.push("region_missing");
        const budgetKey = `${lock.regionId}|${lock.rule}`;
        if (label !== undefined) {
            const used = heldByRegionRule.get(budgetKey) ?? 0;
            if (used >= regionBudget(lock.regionId, lock.rule))
                reasons.push("over_budget");
        }
        if (state.inNoContent(lock.cell[0], lock.cell[1]))
            reasons.push("in_no_content_zone");
        let physical = [];
        let accessCell = null;
        let anchorPoiType = null;
        if (reasons.length === 0 && lock.rule === "world_boss.v1") {
            const origin = lock.arenaOrigin;
            const side = lock.arenaSide;
            const cellIndex = lock.cell[1] * width + lock.cell[0];
            if (origin[0] + side > width || origin[1] + side > height ||
                bundle.bits[cellIndex] !== 1) {
                reasons.push("cell_not_walkable");
            }
            else if (bundle.distanceFromSpawn[cellIndex] === UNREACHABLE) {
                ...
                if (!arenaWalkable)
                    reasons.push("cell_not_walkable");
                else if (anySafe(state, physical) || !admissible(state, physical, buffer))
                    reasons.push("arena_blocked");
            }
        ...
        claim(state.placementMask, physical);
        claim(state.bufferMask, discCells(state, lock.cell[0], lock.cell[1], lock.exclusionRadius));
        const access = accessCell;
    ...
    for (const region of plan.regions) {
        ...
        if (remainingBudget <= 0)
            continue;
        const lockedAnchorIds = new Set(state.placements
    ...
    for (const [regionId, anchors] of [...anchorsByRegion.entries()].sort((a, b) => (a[0] < b[0] ? -1 : 1))) {
        if ((budgetByRegion.get(regionId) ?? 0) > 0)
            continue;
    ...
    for (const placement of state.placements) {
        if (placement.locked && placement.rule === "world_boss.v1") {
            peerFields.push(distanceField(bundle.bits, width, height, [placement.cell[1] * width + placement.cell[0]]));
        }
    }
    for (const region of plan.regions) {
        const heldBosses = heldByRegionRule.get(`${region.id}|world_boss.v1`) ?? 0;
        if (region.budgets.worldBosses - heldBosses <= 0)
            continue;
        // Fresh boss slots skip ids held by locks — the same symmetry the
        // dungeon pass has, so a held lock can never collide a fresh id.
        let slot = 0;
        while (heldIds.has(`placement.world_boss.${region.id}.${slot}`))
            slot += 1;
