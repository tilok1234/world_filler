...
const PROTECTED_SUBDIRS = [
    "src", "tests", "tools", "docs", "fixtures", "consumers", "viewer", "worlds", "recipes",
    "dist", "node_modules", "schemas", ".git", "vendor",
];
export function repoRoot() {
    ...
    while (true) {
        if (existsSync(join(dir, "package.json"))) {
            ...
            if (name === "world-filler")
                return dir;
        }
        const parent = dirname(dir);
        if (parent === dir)
            throw new Error("guard: cannot locate the world-filler repository root");
        dir = parent;
    }
}
function packageName(packageJsonPath) {
    try {
...
function ancestorPackageNames(path) {
    const names = [];
 ...
 */
export function assertOutputRoot(requested) {
    ...
    for (const name of ancestorPackageNames(absolute)) {
        if (name === "worldforge") {
            throw new Error(`guard: refusing output root ${absolute}: it resolves into a WorldForge checkout (read-only upstream)`);
        }
    }
    const root = repoRoot();
    if (absolute === root) {
    ...
    if (isInside(root, absolute)) {
        const first = relative(root, absolute).split(sep)[0];
        if (PROTECTED_SUBDIRS.includes(first)) {
            throw new Error(`guard: refusing output root ${absolute}: ${first}/ is a protected repository tree, use outputs/`);
        }
        return absolute;
    }
    for (const extra of extraOutRoots()) {
        if (absolute === extra || isInside(extra, absolute))
            return absolute;
    }
    throw new Error(`guard: refusing output root ${absolute}: outside world-filler-owned roots ` +
