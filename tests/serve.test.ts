...
describe("director studio API", () => {
  ...
  it("serves analysis maps that agree with the world pack's reference data", async () => {
    ...
    const decodeRuns = (runs: Array<[number, number]>) => runs.reduce((sum, [, count]) => sum + count, 0);
    ...
    const reference = JSON.parse(readFileSync(join(FEN, "walkability.json"), "utf8")) as { grid: string };
  ...
  it("diffs the previous export against the current one, exactly", async () => {
    ...
    assert.equal(
      data.placements.added.length + data.placements.moved.length + data.placements.unchanged,
      total,
      "diff accounts for every current placement",
    );
    assert.ok(
      data.placements.moved.length + data.placements.added.length + data.territories.resized.length + data.territories.added.length > 0,
      "a seed change visibly changes the layout",
    );
